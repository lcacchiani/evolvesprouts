...
describe('AboutUsMyJourney section', () => {
  it('uses migrated overlay/card classes and renders the local journey image', () => {
    ...
    expect(screen.getByText(enContent.aboutUs.myJourney.description)).toBeInTheDocument();

    const firstCard = screen
      .getByText(enContent.aboutUs.myJourney.cards[0].tag)
      .closest('article');
    expect(firstCard).not.toBeNull();
    expect(firstCard?.className).toContain('es-my-journey-card--blue');

    const secondCard = screen
      .getByText(enContent.aboutUs.myJourney.cards[1].tag)
      .closest('article');
    expect(secondCard).not.toBeNull();
    expect(secondCard?.className).toContain('es-my-journey-card--yellow');

    const firstTag = screen.getByText(enContent.aboutUs.myJourney.cards[0].tag);
    expect(firstTag.className).toContain('es-my-journey-tag');

    const thirdCard = screen
      .getByText(enContent.aboutUs.myJourney.cards[2].tag)
      .closest('article');
