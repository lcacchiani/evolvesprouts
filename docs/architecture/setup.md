...
- **Run order:** Run entities in dependency order (`families` / `organizations` → `contacts` → `notes`; `venues` is independent). On non-dry-run imports, the Lambda raises `DependencyNotMet` if a required parent entity has no rows in `legacy_import_refs` yet — import the missing entity first or use dry-run to preview without inserting.
