...
- `CDK_PARAM_FILE` (e.g. `backend/infrastructure/params/production.json`)
  - For Public WWW deploys, include:
    ...
    - `PublicWwwMediaRequestApiBaseUrl` (typically `<FROM_GITHUB_VAR: PUBLIC_WWW_MEDIA_REQUEST_API_BASE_URL>`)
- `NEXT_PUBLIC_WWW_CRM_API_BASE_URL` (for Public WWW builds)
- `PUBLIC_WWW_MEDIA_REQUEST_API_BASE_URL` (execute-api base URL for Public WWW media request proxy routing)
