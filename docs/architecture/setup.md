...
- `CDK_PARAM_FILE` (e.g. `backend/infrastructure/params/production.json`)
  - For Public WWW deploys, include:
    ...
    - `PublicWwwMediaRequestApiBaseUrl` (typically `<FROM_GITHUB_VAR: NEXT_PUBLIC_ADMIN_BASE_API_URL>`)
- `NEXT_PUBLIC_WWW_CRM_API_BASE_URL` (for Public WWW builds)
- `NEXT_PUBLIC_ADMIN_BASE_API_URL` (execute-api base URL for Public WWW media request proxy routing)
