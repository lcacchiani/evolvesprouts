...
- **CDK deploy wiring:** `deploy-backend.yml` maps each of these (and `NEXT_PUBLIC_BUSINESS_PHONE_NUMBER`, `CDK_PARAM_INVOICE_DISPLAY_TIMEZONE`, `CDK_PARAM_INVOICE_PAYMENT_TERMS_DAYS`) into the deploy step `env` so `resolve_value` can substitute `<FROM_GITHUB_VAR: ...>` from `production.json`. If a name is not in the job environment, substitution is empty, the deploy script **omits** that parameter (non-`WafWebAclArn`), and CloudFormation leaves the template default (often `""`) on the Admin Lambda—so invoice PDFs show no business name, address, bank block, or footer text until a deploy with the variables both **set in GitHub** and **listed in the workflow `env` block**.
