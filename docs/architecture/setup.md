...
- `CDK_PARAM_SALES_RECAP_DISPLAY_TIMEZONE` (optional IANA id, e.g. `Asia/Hong_Kong`, for sales recap **Submitted at**; empty uses app default)
...
- **Run order:** `venues` is independent. For membership links, run `families` and/or `organizations` before `contacts` when you need `family_members` / `organization_members` rows. `contacts` can run without prior `families` (contacts insert standalone; membership is optional). Run `notes` after `contacts` (non-dry-run requires `contacts` in `legacy_import_refs`). Use **`dry_run: true`** to get `preview` + `row_details` for PII entities (including planned contact rows).
