...
**Decision:** Customer AR invoice PDFs (`billing-invoice-v15`) use a two-page layout when payment instructions apply (positive total with bank details and/or an HKD FPS QR). The first page ends with terms, a centred line directing readers to the next page, and a bottom footer plus `page/total` page numbers. The second **Payment Options** page lists bank transfer details and/or an FPS logo plus QR (HKD only, when FPS env vars are set and the EMVCo payload builds successfully), using the same payload rules as `apps/public_www/public/scripts/fps-generator.js`. Optional `PUBLIC_WWW_EMAIL` (GitHub `NEXT_PUBLIC_EMAIL`) is interpolated into payment-confirmation copy. Non-HKD invoices, missing FPS configuration, or failed FPS payload builds skip the QR while still emitting bank-only bullets when bank fields are set. Zero or negative totals omit the due date and the entire payment-instructions block.
