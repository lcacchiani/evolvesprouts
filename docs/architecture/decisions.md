...
**Decision:** Customer AR invoice PDFs (`billing-invoice-v16`) use a two-page layout when payment instructions apply (positive total with bank details and/or an HKD FPS QR). The first page ends with terms, a centred bold line directing readers to the next page, and a bottom footer plus `page/total` page numbers. The second **Payment Options** page uses a **Bank Transfer** bullet, an FPS scanning line with only **FPS** in bold, left-aligned payment-confirmation lines, a double-sized FPS mark beside the QR when configured, and a centred bold **Thank you!** closing. Optional `PUBLIC_WWW_BILLING_EMAIL` (GitHub `NEXT_PUBLIC_BILLING_EMAIL`) is interpolated into payment-confirmation copy. Non-HKD invoices, missing FPS configuration, or failed FPS payload builds skip the QR while still emitting bank-only bullets when bank fields are set. Zero or negative totals omit the due date and the entire payment-instructions block.
