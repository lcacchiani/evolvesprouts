...
| `/v1/assets/share/{token}` | GET | API Key (CloudFront origin header) | `EvolvesproutsAdminFunction` | Public bearer-link resolver (302 redirect) with per-asset source-domain allowlist |
