...
| `SALES_RECAP_DISPLAY_TIMEZONE` | IANA timezone id for **Submitted at** in sales recap bodies (CDK sets `Asia/Hong_Kong`; app falls back to the same if unset or invalid). Deprecated alias: `ADMIN_FORM_RECAP_DISPLAY_TIMEZONE` (used only when this is unset). |
...
| `SALES_RECAP_DISPLAY_TIMEZONE` | IANA timezone id for **Submitted at** in media-lead sales recaps (CDK sets `Asia/Hong_Kong`). Deprecated alias: `ADMIN_FORM_RECAP_DISPLAY_TIMEZONE`. |
