...
- Supported entities: `venues`, `families`, `organizations`, `contacts`, `notes`, `link_contact_memberships` (registry in `app.imports.entities`; workflow choice list must stay aligned)
...
- Purpose: parse mysqldump text, write target rows into the unified `notes` table (formerly `crm_notes` before migration `0028_unify_notes_storage`), record `legacy_import_refs` for idempotent re-imports
