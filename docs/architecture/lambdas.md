...
- **Events stack:** `venues`, `families`, `organizations`, `labels`, and `contacts` have no cross-dependency. Then `event_services` (optional deps: `venues`, `organizations`, `labels` — empty ref maps are allowed), `event_instances` (needs `event_services`; optional `venues` for `locations` FK), `event_instance_tags`, `event_enrollments` (PII), `event_discount_codes`. **Partner org fan-out:** a legacy `event` with `organization_id` produces one `service_instance_organizations` row per imported `event_date` instance (partner is stored at instance level in Aurora). **`event_discount_codes`** has no `DEPENDS_ON` guard: scoped rows skip when refs are missing; global codes import anytime; the importer loads `event_services` / `event_instances` ref maps from the database when present. **Timestamps:** mysqldump datetime values without a timezone are treated as UTC (same as other importers); if the source DB was local time, `enrolled_at` may appear shifted — compare against the legacy export, not wall-clock “now”.
