...
- **Dry-run and `contacts`:** Parent UUIDs come **only** from `legacy_import_refs` in the database (loaded from prior **non-dry-run** `families` / `organizations` imports). A dry-run contacts import with empty ref maps reports `skipped_no_dep` for every person row that has a `family_id` — this is expected until those entities are imported. The response may include a `diagnostics` object (counts and a short hint) for operators.
