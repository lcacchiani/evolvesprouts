...
def test_try_resolve_exact_match_single_vendor(mock_session: MagicMock) -> None:
    vendor = SimpleNamespace(
        id=UUID("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"), name="Acme Co"
    )
    result = MagicMock()
...
def test_try_resolve_reverse_when_list_name_shorter_than_parsed(
...
def test_try_resolve_reverse_prefers_longest_vendor_name(
    mock_session: MagicMock,
) -> None:
    v_short = SimpleNamespace(
        id=UUID("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"), name="Canva"
    )
...
def test_try_resolve_reverse_tie_longest_length_returns_none(
    mock_session: MagicMock,
) -> None:
...
def test_try_resolve_fuzzy_skipped_for_short_substring_query(
    mock_session: MagicMock,
) -> None:
    empty = MagicMock()
    empty.scalars.return_value.all.return_value = []
...
def test_try_resolve_fuzzy_skipped_when_only_generic_tokens(
    mock_session: MagicMock,
) -> None:
    empty = MagicMock()
    empty.scalars.return_value.all.return_value = []
