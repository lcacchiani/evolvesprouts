"""PostgreSQL integration: migration ``0047_orgs_slug_to_partner_key`` renames organizations.slug."""
...
def test_0047_renames_organizations_slug_column_and_index() -> None:
    _run_alembic("upgrade", "head")
    _run_alembic("downgrade", "0046_services_slug_to_key")
    ...
    with psycopg.connect(conn_url) as conn:
        cur = conn.execute(
            ...
            WHERE table_schema = 'public' AND table_name = 'organizations'
              AND column_name IN ('slug', 'partner_key')
            """
        )
        cols = {row[0] for row in cur.fetchall()}
        assert "slug" in cols
        assert "partner_key" not in cols

    _run_alembic("upgrade", "0047_orgs_slug_to_partner_key")
    ...
    with psycopg.connect(conn_url) as conn:
        ...
        cur2 = conn.execute(
            ...
            WHERE schemaname = 'public' AND tablename = 'organizations'
              AND indexname = 'organizations_partner_key_unique_idx'
            """
        )
