...
BUCKET_QUERY="Stacks[0].Outputs[?OutputKey=='PublicWwwBucketName']."
...
if [ -z "$BUCKET_NAME" ] || [ "$BUCKET_NAME" = "None" ]; then
  echo "Public WWW bucket output not found for $STACK_NAME"
  exit 1
fi

echo "Syncing Public WWW to s3://$BUCKET_NAME"
aws s3 sync "$BUILD_DIR" "s3://$BUCKET_NAME" --delete

DISTRIBUTION_QUERY="Stacks[0].Outputs[?OutputKey=='PublicWwwDistributionId']."
