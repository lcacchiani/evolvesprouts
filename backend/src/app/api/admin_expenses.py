...
from app.api.admin_request import encode_cursor, parse_body, parse_cursor, parse_uuid, query_param
...
def handle_admin_expenses_request(
    ...
    if len(parts) == 3:
        ...
        if method == "PATCH":
            return _update_expense(event, expense_id=expense_id, actor_sub=identity.user_sub)
        return json_response(405, {"error": "Method not allowed"}, event=event)

    if len(parts) == 4 and parts[3] == "cancel":
        if method != "POST":
            return json_response(405, {"error": "Method not allowed"}, event=event)
        return _cancel_expense(event, expense_id=expense_id, actor_sub=identity.user_sub)

    if len(parts) == 4 and parts[3] == "mark-paid":
        if method != "POST":
            return json_response(405, {"error": "Method not allowed"}, event=event)
        return _mark_expense_paid(event, expense_id=expense_id, actor_sub=identity.user_sub)

    if len(parts) == 4 and parts[3] == "reparse":
        if method != "POST":
            return json_response(405, {"error": "Method not allowed"}, event=event)
        return _reparse_expense(event, expense_id=expense_id, actor_sub=identity.user_sub)
...
def _create_expense(event: Mapping[str, Any], *, actor_sub: str) -> dict[str, Any]:
    ...
    with Session(get_engine()) as session:
        ...
        return json_response(201, {"expense": serialize_expense(refreshed)}, event=event)
    ...
    *,
    ...
    with Session(get_engine()) as session:
        ...
        if payload["status"] is not None and payload["status"] != expense.status:
            ...
            if expense.status == ExpenseStatus.SUBMITTED and expense.submitted_at is None:
        ...
        if refreshed is None:
            raise NotFoundError("Expense", str(expense_id))
        return json_response(200, {"expense": serialize_expense(refreshed)}, event=event)
    ...
    *,
    ...
    with Session(get_engine()) as session:
        ...
        if expense is None:
            raise NotFoundError("Expense", str(expense_id))
        if expense.status == ExpenseStatus.VOIDED:
            raise ValidationError("Voided expenses cannot be marked paid", field="status")
