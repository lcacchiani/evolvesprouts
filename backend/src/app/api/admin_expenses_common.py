...
def serialize_expense(expense: Expense) -> dict[str, Any]:
    ...
    return {
        ...
        "due_date": expense.due_date.isoformat() if expense.due_date is not None else None,
        ...
        "attachments": [
            {
                ...
                "content_type": attachment.asset.content_type if attachment.asset else None,
...
def parse_create_payload(body: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["status"] = parse_optional_status(optional_field(body, "status")) or ExpenseStatus.SUBMITTED
...
def apply_common_fields(expense: Expense, payload: Mapping[str, Any]) -> None:
    """Apply editable common fields on an expense entity."""
    ...
    expense.subtotal = payload["subtotal"]
    expense.tax = payload["tax"]
    expense.total = payload["total"]
    expense.line_items = payload["line_items"]
...
def parse_optional_parse_status(value: Any) -> ExpenseParseStatus | None:
    ...
    try:
        ...
        raise ValidationError("Invalid expense parse status", field="parse_status") from exc
