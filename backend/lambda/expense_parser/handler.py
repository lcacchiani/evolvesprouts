...
def _process_expense(expense_id: str) -> None:
    with Session(get_engine()) as session:
        ...
        if expense is None:
            logger.warning("Skipping parse for missing expense", extra={"expense_id": expense_id})
            return
    ...
    with Session(get_engine()) as session:
        ...
        if refreshed_expense is None:
            return
        refreshed_expense.vendor_name = _pick_text(parsed.get("vendor_name"), fallback=refreshed_expense.vendor_name)
        refreshed_expense.invoice_number = _pick_text(parsed.get("invoice_number"), fallback=refreshed_expense.invoice_number)
        refreshed_expense.invoice_date = _pick_date(parsed.get("invoice_date"), fallback=refreshed_expense.invoice_date)
        refreshed_expense.due_date = _pick_date(parsed.get("due_date"), fallback=refreshed_expense.due_date)
        refreshed_expense.currency = _pick_currency(parsed.get("currency"), fallback=refreshed_expense.currency)
        refreshed_expense.subtotal = _pick_decimal(parsed.get("subtotal"), fallback=refreshed_expense.subtotal)
        refreshed_expense.tax = _pick_decimal(parsed.get("tax"), fallback=refreshed_expense.tax)
        refreshed_expense.total = _pick_decimal(parsed.get("total"), fallback=refreshed_expense.total)
        line_items = parsed.get("line_items")
        if isinstance(line_items, list):
            refreshed_expense.line_items = line_items
        refreshed_expense.parse_confidence = _pick_decimal(parsed.get("confidence"), fallback=refreshed_expense.parse_confidence)
        refreshed_expense.parser_raw = parsed.get("raw") if isinstance(parsed.get("raw"), dict) else {"raw": parsed}
